...
const calculateBookingPrice = async (params) => {
  try {
    const { carId, dateDebut, dateFin, userId, insuranceId, additionalDrivers = [] } = params;
...
const createBooking = async (bookingData) => {
  try {
    ...
    const finalPrice = Math.max(0, pricing.totalPrice - pointsDiscount);
