...
const handlePaymentReturn = async (request, providerName) => {
  try {
    ...
    return {
      ...
      redirect_url: getRedirectUrl(returnResult.status, returnResult.orderId)
    };
...
const handlePaymentNotification = async (request, providerName) => {
  try {
    ...
    if (payment.status === 'COMPLETED' && notificationResult.status === 'PAID') {
    ...
    if (notificationResult.status === 'PAID') {
  ...
  if (status === 'PAID') {
    await prisma.booking.update({
      ...
      data: {
        ...
        mode_paiement: currentPayment.provider
      }
    });
  }
