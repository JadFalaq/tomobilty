...
/**
 * --- SYNCHRONISATION BACKEND (CONSEILS GÉNÉRAUX) ---
 * 1. Utilise un client API centralisé (axios).
 * 2. Définis une variable d'environnement pour l'URL de base.
 * 3. Pour Prisma, assure-toi que tes routes retournent les relations nécessaires (ex: .findMany({ include: { brand: true, images: true } }))
 */
...
// --- DONNÉES DE DÉMO (À REMPLACER PAR DES ETATS useState + useEffect) ---
// BACKEND : Remplace ces constantes par des états que tu rempliras avec useEffect au chargement du composant.
const CATEGORIES = [
...
function CategorySelector({ selectedCat, onSelect }) {
  // BACKEND LINK : Ici, tu devrais utiliser un useEffect pour fetch tes catégories depuis la table 'CarCategory' Prisma.
  // Exemple : useEffect(() => { axios.get('/api/categories').then(res => setCategories(res.data)) }, [])
  return (
...
function AuthPortal({ isOpen, onClose }) {
  ...
  /**
   * BACKEND LINK : Gestion de l'Auth
   * 1. Crée un état local pour les inputs (email, password).
   * 2. Sur onSubmit :
   * - Si mode === 'login' : POST /api/auth/login -> Retourne un Token JWT et les infos User.
   * - Si mode === 'register' : POST /api/auth/register -> Crée une entrée dans la table 'User' Prisma.
   * 3. Stocke le token dans le localStorage ou un cookie sécurisé.
   */
  const handleAuthSubmit = async (e) => {
    e.preventDefault();
    // const response = await axios.post(`/api/auth/${mode}`, formData);
...
function HomeView({ setView, setSelectedCat }) {
  /**
   * BACKEND LINK : Dashboard d'accueil
   * - Fetch TOP_CARS : GET /api/cars/popular (Trié par nombre de réservations dans la table Booking).
   * - Fetch PROMOS : GET /api/promotions (Table Promotion jointe à CarCategory).
   */
...
function CarsPage({ selectedCat, setSelectedCat, setView }) {
  /**
   * BACKEND LINK : Catalogue de Voitures
   * 1. useEffect pour fetch les voitures : axios.get('/api/cars', { params: filters })
   * 2. Sur ton backend Prisma, utilise : prisma.car.findMany({ where: { category_id: selectedCat, transmission: filters.transmission, prix_par_jour: { gte: min, lte: max } } })
   */
  ...
  const filteredCars = useMemo(() => {
    // Si tu fetch via API, cette logique se fera côté Serveur via Prisma.
  ...
  const handleCheckAvailability = async () => {
    /**
     * BACKEND LINK : Vérifier disponibilité
     * GET /api/bookings/check?variante_id=...&start=...&end=...
     * Backend Prisma : prisma.booking.findFirst({ where: { variante_car_id: id, OR: [ { date_debut: { lte: end }, date_fin: { gte: start } } ] } })
     * Si null -> disponible. Sinon -> non disponible.
     */
  ...
  const confirmReservation = async () => {
    /**
     * BACKEND LINK : Créer une réservation
     * POST /api/bookings -> Crée un record dans la table 'Booking' Prisma.
     * N'oublie pas de lier le user_id récupéré lors du login.
     */
    setView('home');
  };
...
function BookingsPage({ setView }) {
  /**
   * BACKEND LINK : Mes Réservations
   * GET /api/user/bookings -> Récupère l'historique de la table 'Booking' pour l'utilisateur courant.
   * Backend Prisma : prisma.booking.findMany({ where: { user_id: currentUserId }, include: { varianteCar: { include: { car: true } }, invoices: true } })
   */
  ...
  const downloadInvoice = async (invoiceId) => {
    /**
     * BACKEND LINK : Facturation
     * GET /api/invoices/:id/download -> Retourne le PDF stocké dans le 'pdf_path' du modèle Invoice Prisma.
     */
...
function SearchSection({ isSticky }) {
  // BACKEND LINK : La recherche peut rediriger vers la page 'cars' avec des query params.
  // window.location.href = `/cars?retrait=${city}&date=${date}`
  return (
