...
const validateBookingData = (req, res, next) => {
  ...
  if (mode_paiement && !['EN_LIGNE', 'EN_AGENCE'].includes(mode_paiement)) {
    errors.push('mode_paiement invalide (EN_LIGNE ou EN_AGENCE requis)');
  }

  // Protection id validation (format only; existence checked in service)
  if (protection_id !== undefined && protection_id !== null) {
    const pid = parseInt(protection_id);
    if (isNaN(pid) || pid < 1) {
      errors.push('protection_id invalide');
    }
